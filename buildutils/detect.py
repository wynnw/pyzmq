...
def detect_zmq(basedir, compiler=None, **compiler_attrs):
    ...
    **compiler_attrs : dict
    ...
    # strip out the linking to zmq, as vers.c gets the version just from the header
    restore_zmq = False
    if 'zmq' in compiler_attrs['libraries']:
        restore_zmq = True
        compiler_attrs['libraries'].remove('zmq')
    ...
    efile = test_compilation(cfile, compiler=compiler, **compiler_attrs)
    if restore_zmq:
        compiler_attrs['libraries'].append('zmq')
